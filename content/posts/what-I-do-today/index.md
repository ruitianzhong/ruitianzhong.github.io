...
+ Excel中的公式其实就好像是一门编程语言，表达力很强
+ COUNTIF 根据条件计数
+ SUMIF & SUMIFS 根据条件求和
+ 快速选中某个范围内的单元格 Shift键
+ 通配符*和？，快速匹配
+ 筛选 + 排序
+ 数据 -> 分列 便于拆分一些有规律的字符串，比如说时间信息
...
+ 今天调研了一下各大公司技术岗位的招聘情况，还是有一些收获的

### 聚类算法

+ 分类问题：已知分类，聚类问题：未知分类
+ 基于距离（系统分析、系统聚类、K-means聚类算法、k++-means聚类算法 ）or 基于密度（DBSCAN）
...
```shell
go mod init zrt.ink/example
go mod edit -replace zrt.ink=../example
go mod tidy 
```

#### Allocation with new & composite literal & make

+ make create slice & map & channel only
...
func safelyDo(x int) (err error) {
 err = nil
 defer func() {
  if e := recover(); e != nil {
   err = e.(RecoverableError)
   fmt.Println("Recoverable Error")
  }
 }()
 do(x)
 return err
...
func do(x int) {
 if x != 42 {
  panic(UnrecoverableError("Unrecoverable Error"))
 } else {
  panic(RecoverableError("Recoverable Error"))
 }
}
...
+ xxxx_test.go
+ Function Name TestXxxx
+ testing.T & testing.Fatalf()
...
### Go Programming Language Notes 1
